<template>
  <div>
    <div class="goods">
      ...
      <shopcart :delivery-price="vendor.deliveryPrice" :min-price="vendor.minPrice" :selected-foods="selectedFoods"
                ref="shopcart"> </shopcart>
    </div>
    <foodDetail @add="addFood" :food="selectedFood" ref="foodDetail"> </foodDetail>
  </div>
</template>
  ...
  export default {
    ...
    created() {
      ...
      this.$http.get('/api/goods').then((response) => {
        ...
        if (response.errno === ERR_OK) {
          ...
          // vue 在更新DOM时是异步的，在nextTick里才进行DOM的更新
          this.$nextTick(() => {
            this._initScroll();
    ...
    methods: {
      selectMenu(index, event) {
        // better_scroll 会派发一个事件，避免和PC的事件重复
      ...
      // 不带下划线的方法可以被web调用，带下划线的方法只能被内部调用
      _drop(target) {
        // 体验优化,异步执行下落动画
        this.$nextTick(() => {
