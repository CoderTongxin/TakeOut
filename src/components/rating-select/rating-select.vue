<template>
  <div class="rating-select">
    ...
    <div class="switch" :class="{'on':onlyShowContent}" @click="toggleContent">
...
<script type='text/ecmascript-6'>
  ...
  export default {
    ...
    computed: {
      positiveCount() {
        let positiveRatings = this.ratings.filter((rating) => {
          return rating.rateType === POSITIVE;
        });
        return positiveRatings.length;
      },
      negativeCount() {
        let negativeRatings = this.ratings.filter((rating) => {
          return rating.rateType === NEGATIVE;
        });
        return negativeRatings.length;
      }
    },
    methods: {
      select(type, event) {
